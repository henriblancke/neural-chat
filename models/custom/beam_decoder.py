...
def batch_gather(params, indices, validate_indices=None,
    ...
    return tf.gather(flat_params, indices_into_flat,
...
class BeamSearchHelper(object):
    ...
    def beam_loop(self, time, cell_output, cell_state, loop_state):
        ...
        symbols_history = flat_batch_gather(past_beam_symbols, parent_refs,
        ...
        next_cell_state = nest_map(
            lambda element: batch_gather(element, parent_refs,
                                         batch_size=self.batch_size,
                                         options_size=self.beam_size),
            cell_state
        )
        ...
        done_symbols = flat_batch_gather(past_beam_symbols, done_parent_refs,
                                         batch_size=self.batch_size,
                                         options_size=self.beam_size)
        ...
        cand_symbols = tf.concat(1, [cand_symbols_unpadded,
                                     tf.fill([self.batch_size_times_beam_size, 1], self.stop_token)])
    ...
    def decode_dense(self):
        emit_ta, final_state, final_loop_state = tf.nn.raw_rnn(self.cell, self.loop_fn,
                                                               scope=self.scope)
